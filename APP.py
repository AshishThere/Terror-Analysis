# -*- coding: utf-8 -*-
"""
Created on Tue Sep  8 18:18:45 2020
...
def load_data():
  ...
  for cols in df.columns.tolist():
        if df[cols].dtype == int:
            if (df[cols].isnull().any(axis = 0)) == True:
                df[cols].fillna(0,inplace=True)
        elif df[cols].dtype == float:
            if (df[cols].isnull().any(axis = 0)) == True:
                df[cols].fillna(0.0,inplace=True)
        elif df[cols].dtype == str:
            if (df[cols].isnull().any(axis = 0)) == True:
...
def create_app_ui():
  ...
  dcc.Tabs(id="Tabs", value="Map",children=[
      dcc.Tab(label="Map tool" ,id="Map tool",value="Map", children=[
          ...
          dcc.Dropdown(
                id='country-dropdown', 
                options=[{'label': 'Select region first', 'value': 'Select region first'}],
                placeholder='Select Country',
                multi = True
                  ),
          dcc.Dropdown(
                id='state-dropdown', 
                options=[{'label': 'Select country first', 'value': 'Select country first'}],
                placeholder='Select State or Province',
                multi = True
                  ),
          dcc.Dropdown(
                id='city-dropdown', 
                options=[{'label': 'Select state first', 'value': 'Select state first'}],
...
def set_country_options(region_value):
    ...
    return [{'label':m , 'value':m} for m in sorted(option)]
